...
@InstallMethod( PresentationMorphism,
  ...
  function( category, source, matrix, range )
    ...
    if (@not IsIdenticalObj( CapCategory( source ), category ))
        
        Error( "<source> does not lie in <category>" );
        
    end;
    
    if (@not IsIdenticalObj( CapCategory( range ), category ))
        
        Error( "<range> does not lie in <category>" );
        
    end;
    
    if (@not IsIdenticalObj( UnderlyingRing( category ), HomalgRing( matrix ) ))
        
        Error( "matrix can not present a morphism between these objects" );
        
    end;
    
    if (NrRows( matrix ) != NrColumns( UnderlyingMatrix( source ) ))
        
        Error( "the number of rows of the given matrix is incorrect" );
        
    end;
    
    if (NrColumns( matrix ) != NrColumns( UnderlyingMatrix( range ) ))
...
@InstallMethod( PresentationMorphism,
  ...
  function( category, source, matrix, range )
    ...
    if (NrColumns( matrix ) != NrRows( UnderlyingMatrix( source ) ))
        
        Error( "the number of columns of the given matrix is incorrect" );
        
    end;
    
    if (NrRows( matrix ) != NrRows( UnderlyingMatrix( range ) ))
...
@InstallMethod( StandardGeneratorMorphism,
  ...
  function( module_presentation, i_th_generator )
    ...
    if (i_th_generator > number_of_generators)
      
      Error( @Concatenation( "number of standard generators is ", 
                            StringGAP( number_of_generators ), ", which is smaller than ", StringGAP( i_th_generator ) ) );
